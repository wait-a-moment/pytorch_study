...
train, val, test = torchtext.datasets.LanguageModelingDataset.splits(path="./data/text8", train="text8.train.txt",
...
#     print(batch.text[:, 2].data)
#     print("data:" + " ".join([TEXT.vocab.itos[i] for i in batch.text[:, 2].data]))
#     print("target: " + " ".join([TEXT.vocab.itos[i] for i in batch.target[:, 2].data]))
...
for i in range(100):
    ...
    word_idx = torch.multinomial(word_weights, 1)[0]  # torch.multinomial(input, num_samples,replacement=False, out=None) → LongTensor 作用是对input的每一行做n_samples次取值，输出的张量是每一次取值时input张量对应行的下标。
    # 作为下一次的输入
    input.fil_(word_idx)
